...
/**
 * @param {number[]} input
 * @param {number} targetSubarraySize
 * @return {number[]}
 */
var resultsArray = function (input, targetSubarraySize) {
    if (targetSubarraySize === 1) {
        return input;
    }

    const NOT_FOUND = -1;
    const powerOfSubarraysWithTargetSize = new Array(input.length - targetSubarraySize + 1);
